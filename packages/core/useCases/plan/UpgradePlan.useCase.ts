...
export class UpgradeUseCase {
  ...
  async execute(companyId: number, clientId: string, productIds: string[]): Promise<Plan[] | null> {
    const upgradePlanResponse = await this.planService.upgradePlan(companyId, clientId, productIds);
  ...
  private plansWithDiscount(plans: Plan[], planItems: PlanItem[]) {
    ...
    return plans.map(plan => {
      ...
      const valueInPercentToSub = productRepeated.reduce(
        (acc, product) => acc + this.getPlanItemByProcutId(product.product_id, planItems), 0
      );
