...
export class PlanViewerWithProductsRepository {
  ...
  async view(partnersId: number[], planId: number): Promise<Plan | null> {
    const result = await this.db
      ...
      .where(
        and(
          inArray(planPartner.id_parceiro, partnersId),
    ...
    const planCompleted = await this.getPlanRelactions(
      result[0],
