...
@injectable()
export class OrderCreatedViewerRepository {
  ...
  async viewOrderCreated(
    ...
    return enrichPromises as ViewOrderCreatedResponseDto[];
  }
  ...
  private async fetchOrderPlans(tokenKeyData: ITokenKeyData, orderId: string) {
    const result = await this.db
      .select({
        ...
        status: plan.status,
        visible_site: sql<boolean>`CASE WHEN ${plan.visivel_site} = ${PlanVisivelSite.YES} 
                                            THEN true
                                        ELSE false
                                    END`,
