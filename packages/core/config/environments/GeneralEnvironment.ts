...
export class GeneralEnvironment {
  ...
  private readonly APP_URL_WEBHOOKS: string | undefined;
  ...
  constructor() {
    ...
    this.APP_URL_WEBHOOKS = process.env.APP_URL_WEBHOOKS;
  ...
  public get appUrlWebhooks(): string {
    if (!this.APP_URL_WEBHOOKS) {
      throw new InvalidConfigurationError("APP_URL_WEBHOOKS is not defined.");
    }

    return this.APP_URL_WEBHOOKS;
  }
  ...
  public get publicSplitRuleId(): number {
    if (!this.PUBLIC_SPLIT_RULE_ID) {
      throw new InvalidConfigurationError(
        "PUBLIC_SPLIT_RULE_ID is not defined."
      );
    }
